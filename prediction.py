...
def alarm_pred(dataframe):
    ...
    #     print(h)
        ...
        innit_AlarmDateCount.load_input(df_in_alarm[0],output_df_true)
        innit_AlarmDateCount.getlastalarmcount()
        dfMA_train = innit_AlarmDateCount.appendrecord()
        innit_AlarmDateCount.gettimeinterval(dfMA_train)
        dfMA_train = innit_AlarmDateCount.Map_RuleId(df_in_alarm_frame,output_df_true,df_cat3,rulejson)
        h+=1

    #Generating alarm action report
    report = dfMA_train.drop(['Rule_Ticket'],axis=1)
    report.to_excel("Alarm Action Report.xls")

    df_duplicate =dfMA_train[dfMA_train['Rule_Ticket']==1]
    print(df_duplicate )
    print("----------------------------------------")
    ...
    for i in range(len(df_duplicate)):
        print('------------------------------------------------')
        print(df_duplicate)
        d_count=df_duplicate['Rule_Action'][i].str.contains('|1').sum()
        print(d_count)
        if(d_count > 0):
            appenddata = appenddata.append(df_duplicate[i])
            print("---------------------")
            print(appenddata.empty)

    # if(appenddata.empty == False):
    #     print('!!!!!!!!!!!!!!!!')
        # appenddata.to_excel('Alarm Duplication Report.xls')
    ...
    # shutil.move('False_Alarm_Report', '/output')
